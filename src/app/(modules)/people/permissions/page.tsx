...
export default function PermissionsPage() {
  ...
  const teamLookup = useMemo(() => {
    return operationsTeams.reduce<Record<string, string>>((acc, t) => {
      acc[t.key] = t.label;
      return acc;
    }, { logistics: "Giao vận" });
  }, [operationsTeams]);
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Phân quyền nhân viên</h2>
          <p className="text-sm text-slate-600">Gán nhân viên vào tổ sản xuất, quản lý ca trực và quyền truy cập để đảm bảo tuân thủ quy trình.</p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-900">Bảng phân tổ và quyền hạn</h3>
          <div className="flex flex-wrap items-center gap-2 text-xs text-slate-600">
            <button className="rounded-full border border-emerald-300 bg-emerald-50 px-3 py-1 uppercase tracking-wide text-emerald-700 hover:bg-emerald-100">Thêm nhân viên</button>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-900">Quy tắc chung</h3>
          <ul className="mt-3 space-y-2 text-sm text-slate-700">
            <li>- Nhân viên chỉ xem các màn hình được gán theo tổ.</li>
            <li>- Tổ trưởng có quyền duyệt checklist và kích hoạt bàn giao.</li>
            <li>- Giao vận chỉ được truy cập dữ liệu sản xuất.</li>
          </ul>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-900">Checklist onboarding nhanh</h3>
          <ol className="mt-3 space-y-2 text-sm text-slate-700 list-decimal pl-4">
            <li>Thu thập thông tin cơ bản và ca trực.</li>
            <li>Chọn tổ, mức độ định danh và quyền thao tác tương ứng.</li>
            <li>Gửi email hướng dẫn hoặc in thẻ QR truy cập.</li>
            <li>Nhắc kỹ quy trình an toàn và bảo mật dữ liệu.</li>
          </ol>
          <button className="mt-4 inline-flex items-center justify-center rounded-full border border-emerald-300 bg-emerald-50 px-4 py-2 text-xs font-semibold uppercase tracking-wide text-emerald-700 hover:bg-emerald-100">Bắt đầu onboarding</button>
        </div>
      </div>
    </div>
  );
